...
const LearnContentPage: NextPage<ContentPageProps> = ({
  ...
  const sortedSubContents = subContents.sort((a, b) =>
    dayjs(a.frontMatter.created_at).isAfter(dayjs(b.frontMatter.created_at))
      ? 1
      : -1
  );
  ...
  const canonicalUrl = `https://maalik.dev/learn/${content?.slug}`;

  return (
    <>
      <NextSeo
        title={`Learn ${title} - Malik Bagwala`}
